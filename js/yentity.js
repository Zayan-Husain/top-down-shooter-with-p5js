...
console.log(t.sprite.animation)	
  ...
  sy(y2)
  {
	...
	angle_between(x2,y2){return -Math.atan2(this.x-x2,this.y-y2);}//end angle_between
	angle_betweenr(x2,y2){return -Math.atan2(this.x-x2,this.y-y2)* 180 / Math.PI;}//end angle_between
	
	move_to(target)
	{
		var t = this;
		if(t.x == target.x || t.y == target.y){return;}
		if(t.x<target.x){t.move_by(t.speed,0);}
		if(t.x>target.x){t.move_by(-t.speed,0);}
		if(t.y<target.y){t.move_by(0,t.speed);}
		if(t.y>target.y){t.move_by(0,-t.speed);}
	...
	{
		var t = this;
		var a = t.angle_betweenr(target.x,target.y);
